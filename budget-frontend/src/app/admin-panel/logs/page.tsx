    ...
    <main className={styles.container}>
      ...
      <div className={styles.tableWrapper}>
        <table className={styles.logsTable}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className={styles.infoCell}>Ładowanie...</td></tr>
            ) : logs.length === 0 ? (
              <tr><td colSpan={6} className={styles.infoCell}>Brak logów</td></tr>
            ) : (
              logs.map((log, index) => (
                <tr key={`${log.objectId}-${index}`}>
                  <td className={styles.dateCell}>{new Date(log.creationDate).toLocaleString()}</td>
                  <td><span className={styles.eventTypeBadge}>{log.eventType}</span></td>
                  <td>{log.objectType}</td>
                  <td>{log.userEmail}</td>
                  <td className={styles.dataCell}>
                    <div className={styles.truncate} title={log.before}>{log.before || "-"}</div>
                  </td>
                  <td className={styles.dataCell}>
                    <div className={styles.truncate} title={log.after}>{log.after || "-"}</div>
                  </td>
                </tr>
              ))
            )}
        ...
        <div className={styles.pagination}>
          ...
          <div className={styles.pageControls}>
            ...
            <span className={styles.pageLabel}>Strona {currentPage} z {totalPages}</span>
            <button 
              className={styles.pageBtn} 
              onClick={() => setCurrentPage(c => c + 1)} 
